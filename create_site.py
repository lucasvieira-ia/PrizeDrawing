...
def processar_dados_totais():
    ...
    # 2. Análise Concurso por Concurso (Do primeiro ao último)
    draws_list = []
    ...
    for i, row in df_reset.iterrows():
        ...
        if i > 0:
            ...
            intersection = set(current_numbers).intersection(previous_numbers)
            repeats_output = (
                f"{len(intersection)} número(s) -> {sorted(list(intersection))}"
                if intersection
                else "Nenhum"
            )
    ...
    # Exporta os dados brutos estruturados para JSON dentro do HTML
    return json.dumps(draws_list)


# Gerar a string JSON com os dados
dados_json = processar_dados_totais()

# Ler o template original do seu HTML
with open("templates/index.html", "r", encoding="utf-8") as f:
    html_template = f.read()

# Injetar os dados dentro de uma tag <script> no HTML
html_final = html_template.replace(
    "<!-- DATA_INJECTION_PLACEHOLDER -->", f"const SorteiosData = {dados_json};"
)

# Salvar como index.html na raiz (pronto para o GitHub Pages)
with open("index.html", "w", encoding="utf-8") as f:
    f.write(html_final)

print("✨ Arquivo index.html estático gerado com sucesso para o GitHub Pages!")
