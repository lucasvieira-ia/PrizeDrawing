...
def analyze_global_frequencies(df: pd.DataFrame, ball_columns: list):
    """Calculates and returns most/least frequent numbers as dictionaries."""
    all_balls = df[ball_columns].values.flatten()
    number_counts = pd.Series(all_balls).value_counts()

    most_frequent = number_counts.head(5)
    remaining_numbers = number_counts.drop(most_frequent.index)
    least_frequent = remaining_numbers.tail(5)

    return most_frequent.to_dict(), least_frequent.to_dict()


def analyze_global_parity_percentage(df: pd.DataFrame, ball_columns: list):
    """Calculates and returns parity percentages for the interface."""
    all_balls = df[ball_columns].values.flatten()
    ...
    return {
        ...
        "evens_pct": round((evens_count / total_numbers) * 100, 2),
        "odds_pct": round((odds_count / total_numbers) * 100, 2),
    }


def analyze_draw_by_draw(df: pd.DataFrame, ball_columns: list):
    """Iterates through each draw and returns data mapped as a list of dicts."""
    draws_list = []
    ...
    for i, row in df_reset.iterrows():
        ...
        if i > 0:
            ...
            repeats_output = (
                f"{qty_repeats} número(s) -> {sorted(list(intersection))}"
                if qty_repeats > 0
                else "None"
            )
        ...
        draws_list.append(
            {
                ...
                "numbers": sorted(list(current_numbers)),
                "evens": evens,
                "odds": odds,
...
# ==========================================
# FLASK INTERFACE CORE ROUTE
# ==========================================
@app.route("/", methods=["GET", "POST"])
def index():
    # Pega o filtro selecionado no seletor HTML ("all", 5, 10, 20)
    user_limit = request.form.get("limit", "all")

    # Executa a esteira modular que limpa e corta o DataFrame
    dataframe, ball_cols = load_and_clean_data(RAW_DATA)
    filtered_df = filter_dataset(dataframe, user_limit)

    # Coleta as saídas tratadas das funções
    most_freq, least_freq = analyze_global_frequencies(filtered_df, ball_cols)
    parity_stats = analyze_global_parity_percentage(filtered_df, ball_cols)
    draws_data = analyze_draw_by_draw(filtered_df, ball_cols)

    # Renderiza a página passando as variáveis exatamente iguais às do HTML
